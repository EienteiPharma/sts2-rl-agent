...
def make_masked_env(
    ...
    *,
    ...
    """Pickle-friendly factory: SubprocVecEnv when ``--n-envs > 1``.

    Each worker still hang-protocol Jev on the live RunEnv half.
    """
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Parallel mix envs. >1 uses SubprocVecEnv of MixedHangLoadoutEnvMaker "
            "(each worker still hang-protocol Jev on the RunEnv half). "
            "To keep TypeSafe off learn(), collect then train_combat_from_buffer.py."
        ),
    )
