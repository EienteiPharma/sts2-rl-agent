...
**Track 2 (`bh_assist` + turn-plan):** Jev hints + `combat_turn_plan_choice`; **ban** `combat_step_choice`. Hang zip stays `bh_v1`. Pre-train assist eval: A=`jev-turn` vs B=`jev-turn`+assist; smoke `workers=1`, formal `workers=4`; effectiveness **overall ≥ +3pp and Boss ≥ +2pp** (B−A). Collapse vs hang ppo A unchanged. See `docs/BH_ASSIST_CONTRACT.md` § Pre-train eval gates.
