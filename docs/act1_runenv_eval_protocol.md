...
- `--map-lowhp on|off`（默认 **`on`**，hang 开）。`hp_pressure>=2.0`（Score 缺失时用本地 `max_hp/hp`）且合法 MAP 含 `SHOP`/`REST_SITE` 时，low-conf / API 错 / choice 不合法 **不**抽 monster/elite，只在 shop/rest 里重抽（`map_lowhp_random`）；高压下 Jev 自信选了 fight 则改 rest-then-shop（`map_lowhp_safe`）。仅剩 fight 时仍全池随机。`--jev off` 的全合法随机不变。阈值常量 `MAP_LOWHP_PRESSURE = 2.0`。
