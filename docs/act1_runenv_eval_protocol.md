...
- `--map-lowhp-soft-b on|off`（默认 **`on`**；可开关 `--map-lowhp-soft-b off`）。在 `hp_pressure>=2.0` 且分支前方有精英或 Boss 时，低置信度/错误决策 soft-prefer 避开危险点走 safe 点（`map_lowhp_soft_b`）。评测按步累计 `map_lowhp_soft_b_n`。主线止于 `58db7d0`（4 层 / 1.5 扩展版 `4c85dbd` 因胜率倒退已废弃）。
