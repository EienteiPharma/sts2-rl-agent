...
- `--jev-event off|on`（默认 **`off`**）。仅 hierarchical + `--jev on`。默认普通 EVENT 合法随机（`jev_event_off_random`），**不改**既有 MAP/REST/CARD 表。检出的 Neow 仍走 `neow_boon`（Choice ≥0.65；REST 0.50 / assist 不套 Neow）。`--jev-phases map,rest,card,event` 等价于 `--jev-event on`。`--jev-neow` 默认 **on**（不跟 `--jev-event`）。契约：`docs/JEV_EVENT_NEOW_CONTRACT.md`。
