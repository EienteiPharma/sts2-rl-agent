...
- `--map-lowhp-soft-b on|off`（默认 **`on`**；可开关 `--map-lowhp-soft-b off`）。在 `hp_pressure>=1.5` 且分支前方 4 层内有精英或 Boss 时，低置信度/错误决策 soft-prefer 避开危险点走 safe/非危险点（`map_lowhp_soft_b`）。评测按步累计 `map_lowhp_soft_b_n`。
