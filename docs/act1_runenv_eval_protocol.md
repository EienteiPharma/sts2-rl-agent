...
- Live 调用读 `TYPESAFE_API_KEY`（`sts2_env/eval/jev.py` LiveJevClient）。可选池：`TYPESAFE_API_KEY_2`… 或 `TYPESAFE_API_KEYS`（逗号 / JSON list）。403/1010 换下一把 key + 短 backoff，**不**把 MAP/CARD 改随机。密钥从 env / box-secrets / Cursor secret-request 注入，**不要写进聊天**。缺失时按 error 回退，不改 combat 路径。TODO(Surplus/Jev) 可替换该 adapter，不必改 eval 脚本。
