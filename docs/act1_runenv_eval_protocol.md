...
- `--map-lowhp on|off`（默认 **`on`**，hang 开，**v2 hard-select**）。`hp_pressure>=2.0`（Score 缺失时用本地 `max_hp/hp`）且合法 MAP 含 `SHOP`/`REST_SITE` 时，**无论** Jev 自信与否都执行 rest-then-shop（`map_lowhp_hard`）。仅剩 fight 时仍全池随机。`--jev off` 的全合法随机不变。阈值常量 `MAP_LOWHP_PRESSURE = 2.0`。评测按步累计 `map_lowhp_hard_n`，stderr 打 `map_lowhp_hard ...`。
