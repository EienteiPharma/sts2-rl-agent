...
- `--map-lowhp-soft-b on|off`（默认 **`off`**；仅供实验 opt-in `--map-lowhp-soft-b on`）。开启时在 `hp_pressure>=2.0` 且分支前方有精英或 Boss 时，低置信度/错误决策 soft-prefer 避开危险点走 safe 点（`map_lowhp_soft_b`）。评测按步累计 `map_lowhp_soft_b_n`。主线止于 `58db7d0`/`dcc44b3` 窄口径（4 层 / 1.5 扩展版 `4c85dbd` 因胜率倒退已废弃）。默认关。
...
- 战斗步：从 `RunManager.get_combat_state()` 取 `CombatState`，`encode_observation(combat)` + combat `get_action_mask`，`MaskablePPO.predict`，再把 combat action index 映射到 RunEnv combat slice（layout offset 0）。**Hang 战斗仍是 hung PPO `bh_v1`（`--combat-policy ppo`，默认）。** `--combat-policy jev` 是 **experimental** failed-bypass（HOLD 两次崩盘，第二次 `ecd0073` B 7.8/15.6/Boss 0.0，见 `docs/COMBAT_JEV_HOLD_FAIL.md`），**不是** hang / **不是**下一条主线；旗标保留、默认仍 ppo。
- 非战斗步：
